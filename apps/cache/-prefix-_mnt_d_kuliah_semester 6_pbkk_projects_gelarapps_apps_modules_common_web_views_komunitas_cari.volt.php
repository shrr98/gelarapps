...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
		...
		<title>GelarApps - Cari Komunitas</title>
	</head>
	<body>
    ...
    <h1 align="center" style="font-size: 30px;">Cari Komunitas</h1>
     ...
     <div class="jumbotron">
        ...
        <div class="card-group">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title"><a href="<?= $this->url->get('komunitas/kategori/' . $item->id) ?>"><?= $item->kategori ?></a></h5>
            </div>
            <?php } ?>
          </div>
    </div>
   ...
   <!--  <div>
        <ul>
            <?php foreach ($items as $item) { ?>
                <li>
                    <h5>
                        <a href="<?= $this->url->get('komunitas/kategori/' . $item->id) ?>"><?= $item->kategori ?></a>
                    </h5>
                </li>
            <?php } ?>
        </ul>
